...
**Tier 1: landed.** Beautiful local demo. Express + `public/*.js`, no build. Load compiled `fly-cx.json` (47 / 280, CC-BY Janelia `male-cns:v1.0`). Sliders for K and N. Perspective stack of the heading ring. Nacre on activity. Order, corr, residual. Verse. H hides chrome. Click the ring to set a target. The stack seeks it. Score is `(1 + cos(error)) / 2`. J kicks so stacks desync and must reacquire. A walking target is the idle verb. Telemetry names the live circuit (EPG / PEN / PEG / Delta7 / EL). A thought log is an autopsy of regime, score, and error, not language as the mind. A first gesture arms a compass tone (heading is pitch, score is the interval, residual is roughness). M mutes. Seven field notes advance only when the visitor steers, kicks, sees a real residual, and sees analog interference. They name the ring attractor, corollary discharge, and Miller 2026 interference. They do not name a mind. The dock does not eat the ring. After a gesture, `/jev` asks TypeSafe Jev (or a local typed twin) Choice/Score/Noul questions on the stack state and the hint branches on those answers. Residual after each of the K W+T passes is a depth tape. Jev also names the live dynamical fact (`moment`) and whether both N and K were needed (`emergent`). That is an echo, a wave, or a lock. It is not a mind. The ring is colored by named subgraph (EPG / PEN / PEG / Delta7 / EL) and the atlas line is that subgraph's job. The front ring draws W's edges and T's attention. Telemetry `mix` is the share of attention that does not sit on a synapse. T is a mixer, not a second connectome. Reafference filaments are per-cell |top-before minus floor-after|. Warmth is a miss. The echo line says echo or miss. It does not say mind. Set `TYPESAFE_API_KEY` to use the live model. Do not generate text. Do not call Jev before a gesture.
